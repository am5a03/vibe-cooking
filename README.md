...
CI runs these checks, validates the Drizzle migration journal/snapshots, a high-severity dependency-audit gate, and a Worker packaging dry run. It uploads `dependency-lock` and `kitchen-browser-results` artifacts. Review and commit the package-lock.json generated by npm install to pin transitive dependencies; it is not yet committed on this branch. Deployment remains guarded and manual.

## Boundaries

This is the connected cookbook slice of MealSpin v2, not the full remix/prep-board engine. Automatic recommendations, remix links, shared prep sessions, images and public accounts are deferred. No production resources or migrations are created automatically by this branch.
