...
function Unlock({ expired, onUnlock }: { expired: boolean; onUnlock: () => void }) {
  ...
  return <main className="unlock-layout">
    <div className="unlock-story"><span className="eyebrow">A kitchen of your own</span><h1>Good food.<br/><em>Your way.</em></h1><p>Keep the recipes you love.<br/>Make room for something new.</p><div className="story-mark"><Leaf size={118} strokeWidth={0.8}/></div></div>
    <section className="unlock-card"><LockKeyhole size={26}/><span className="eyebrow">Vibe Cooking / Personal kitchen</span><h2>{expired ? 'Welcome back.' : 'Come on in.'}</h2>
      <p>{expired ? 'Unlock again to continue. Unsaved changes are kept in this tab.' : 'Unlock your private recipe studio with the API_TOKEN you configured on the server.'}</p>
      <form onSubmit={submit}><label className="field"><span>Private kitchen key</span><input type="password" autoComplete="current-password" required minLength={32} maxLength={256} value={key} onChange={(e) => setKey(e.target.value)} placeholder="Paste your private key"/></label><ErrorBox message={error}/><button type="submit" className="button primary full" disabled={busy}>{busy ? 'Unlocking…' : 'Unlock my kitchen'}<ArrowRight size={17}/></button></form>
      <p className="fineprint">No registration. The app does not save your key in browser storage. Your session lasts up to eight hours.</p>
      <details className="setup-help"><summary>First time here?</summary><p>Apply the new browser-session migration with <code>npm run db:migrate:local</code>. Keep using your existing <code>API_TOKEN</code>; no second secret is required. For a remote instance, apply reviewed remote migrations instead.</p></details>
    </section>
  </main>;
}
