...
import { FlavorProvider } from './flavor-context';
import { FlavorLibrary } from './flavor-library';
import { PreferencesPanel } from './preferences';

function routeValue() {
  const route = window.location.hash.slice(1);
  return /^(discover|all|saved|preferences|flavours|new|(?:recipe|favourite|edit|duplicate|variations)\/[A-Za-z0-9_-]+)$/.test(route) ? route : 'discover';
}
export function Studio() {
  ...
  useEffect(() => {
    async function change() {
      ...
      if (['all', 'saved', 'preferences', 'flavours', 'new'].includes(next)) setDiscovery(null);
      setRoute(next);
    }
  ...
  return <ConfirmationContext.Provider value={confirmations}><div className="kitchen-app min-h-screen bg-background text-foreground">
    ...
    {started && <div hidden={locked}><FlavorProvider><KitchenContext.Provider value={{ entries, refreshIngredients, go, setDirty, discovery, setDiscovery }}>
