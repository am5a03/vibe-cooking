    ...
    <header className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 border-b px-5 py-[18px] sm:px-6 lg:flex-nowrap lg:gap-6 lg:px-10 lg:py-6">
      ...
      >
        ...
        <span className="text-[21px] font-bold leading-tight text-foreground sm:text-[23px]">
          Vibe Cooking
          <span className="mt-0.5 block text-[9px] font-normal uppercase tracking-[0.2em] text-muted-foreground">The personal kitchen</span>
        </span>
      </Button>
      {unlocked && (
        <>
          <nav aria-label="Main navigation" className="order-3 grid w-full grid-cols-4 gap-1 lg:order-none lg:flex lg:w-auto lg:gap-2">
            {navigation.map(({ route, label, icon: Icon }) => (
              <Button
                ...
                className={cn(
                  ...
                  view === route ? 'bg-secondary font-semibold text-primary' : 'text-muted-foreground',
                )}
              >
          ...
          <Button type="button" variant="outline" onClick={onLock} disabled={locking} aria-busy={locking} className="h-11 shrink-0 gap-2 bg-transparent px-3 text-xs text-primary">
