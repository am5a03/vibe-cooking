  ...
  return (
    <main className="unlock-layout">
      <div className="unlock-story">
        ...
        <div className="story-mark" aria-hidden="true"><Leaf size={118} strokeWidth={0.8} /></div>
      </div>
      <Card role="region" aria-labelledby={headingId} className="min-w-0 gap-0 rounded-[17px] p-[26px] shadow-[0_10px_40px_#1a2e1e06] min-[681px]:p-[38px]">
        <CardHeader className="gap-0 p-0">
          <LockKeyhole className="mb-6 size-[26px] text-primary" aria-hidden="true" />
          <Badge variant="secondary" className="mb-3 max-w-full whitespace-normal rounded-md px-2 py-1 text-[10px] font-semibold tracking-wide">
          ...
          <CardDescription className="mb-5 text-[13px] leading-relaxed">
            {expired ? 'Unlock again to continue. Unsaved changes are kept in this tab.' : 'Unlock your private recipe studio with the API_TOKEN you configured on the server.'}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          <form onSubmit={submit} aria-busy={busy}>
            <Field className="mb-[18px] gap-2" data-invalid={error ? true : undefined}>
              <FieldLabel htmlFor={fieldId} className="text-xs font-semibold">Private kitchen key</FieldLabel>
              <Input
                ref={input}
                id={fieldId}
                type="password"
            ...
            <Button type="submit" disabled={busy} className="h-11 w-full gap-2 text-[13px] font-semibold">
              ...
              {busy ? <LoaderCircle className="size-[17px] animate-spin" aria-hidden="true" /> : <ArrowRight className="size-[17px]" aria-hidden="true" />}
            </Button>
          </form>
          <span role="status" aria-live="polite" className="sr-only">{busy ? 'Unlocking your kitchen…' : ''}</span>
          ...
          <details className="mt-6 border-t pt-[18px] text-xs text-muted-foreground">
            <summary className="cursor-pointer rounded-sm text-primary outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background">First time here?</summary>
            <p className="mt-4 leading-relaxed">
              Apply the new browser-session migration with <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">npm run db:migrate:local</code>.
              {' '}Keep using your existing <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">API_TOKEN</code>; no second secret is required.
